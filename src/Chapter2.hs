...
firstHalf [] = []
firstHalf [x] = [x]
firstHalf xs = take (fst $ divMod (length xs) 2) xs
...
-}
takeEven :: [a] -> [a]
takeEven xs = reverse $ go 0 [] xs
  where
    go :: Int -> [a] -> [a] -> [a]
    go _ acc [] = acc
    go pos acc (x:xs') = 
      let 
        isEvenPosition = mod pos 2 == 0
        newAcc = if isEvenPosition then x:acc else acc
      in
        go (pos+1) newAcc xs'
